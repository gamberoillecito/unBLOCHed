...
- [X] 🖼**Image export**: You can export the 3D scene as a high resolution PNG image with a single click. (_Hamburger menu in the 3D scene_ $\rightarrow$ _Save Image_)
  - [X] 🖨️**Paper mode downloads**: If you want to print the image on paper, select _Paper Mode_ in the hamburger menu for better results.
...
- [X] 🎧**Noise**: Noise simulation is implemented using the "operator sum representation". Different noise channels can be applied and tuned with a simple UI similar to the quantum gates. 
