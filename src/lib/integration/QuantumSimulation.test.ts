...
import { Xgate, Ygate, Zgate, Hgate, RZgate } from '../data/matrices';
...
describe('Quantum Simulation Integration Tests', () => {
	describe('Complete Quantum State Evolution', () => {
		test('should evolve |0⟩ → H → X → H → |1⟩ correctly', () => {
			// Start with |0⟩ state
			...
			// But actually: (|0⟩ + |1⟩)/√2 → (|1⟩ + |0⟩)/√2 = |+⟩
			state.apply_gate(Xgate);
			expect(state.blochV[0]).toBeCloseTo(1, 1); // Still on x-axis

			// Apply Hadamard again: should give |1⟩
			state.apply_gate(Hgate);
			expect(state.blochV[1]).toBeCloseTo(-1, 1); // z-component = -1 for |1⟩
		});
	...
	describe('Parameterized Gate Evolution', () => {
		test('should handle continuous rotation evolution', () => {
			...
			const param = new MatrixParam('theta', '0', '\\theta', true);
			...
			for (const angle of angles) {
				const testState = new DensityMatrix(
					[
						['1', '0'],
						['0', '0']
					],
				...
				testGate.setParameterLatex('theta', angle.toString());
		...
		test('should handle adiabatic evolution simulation', () => {
			...
			const param = new MatrixParam('t', '0', 't', true);
			const timeGate = new GateMatrix(
				[
					['cos(t)', '-sin(t)'],
					['sin(t)', 'cos(t)']
				],
			...
			for (let i = 1; i < timeSteps.length; i++) {
				const evolGate = timeGate.clone();
				evolGate.setParameterLatex('t', (timeSteps[i] - timeSteps[i - 1]).toString());
